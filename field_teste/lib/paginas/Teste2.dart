...
class _ColetandoState extends State<Coletando> {
    ...
    try {
      if (widget.documentSnapshot.exists) {
        ...
        if (coleta != null && coleta.containsKey('coleta')) { // Verificar se 'coleta' está presente no documento
          ...
          setState(() {
            parcelas = Map<String, List<dynamic>>.from(dadosColeta['parcelas']);
  ...
  String? encontrarParcela() {
    if (tratamentos.isNotEmpty && repeticao.isNotEmpty && parcelas.isNotEmpty) {
      ...
      floatingActionButton: Stack(
        children: [
          ...
          Positioned(
            ...
            child: FloatingActionButton(
              onPressed: () {
                print(celulas['Temperatura-A-1']);
                print(celulas['Temperatura-A-2']);
              },
              backgroundColor: base,
